...
def render_gdelt(gdelt: dict) -> str:
    if gdelt.get("status") != "ok":
        ...
        return (
            "A fonte GDELT não respondeu corretamente nesta coleta. "
            f"Erro registrado: `{error}`.\n"
        )
    ...
    lines = [
        f"A consulta ampla retornou **{count} artigo(s)** na janela configurada.",
        "",
        "> **Importante:** zero resultados significa apenas que esta consulta não encontrou correspondências; "
        "não significa ausência de eventos no conflito.",
        "",
    ]
...
def build_report(payload: dict, source_file: Path) -> str:
    ...
    report = f"""# Cyber Conflict Watch — Daily Brief — {date_label}
